...
def emit_human_results(results: list[dict[str, Any]]) -> None:
    for result in results:
        print(
            f"{'valid' if result['valid'] else 'invalid'}: {result['path']}",
            file=sys.stderr,
        )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate MKF/OKF v0.2 concept frontmatter."
    )
    parser.add_argument(
...
def main(args: argparse.Namespace) -> int:
    ...
    if args.json:
        print(
            json.dumps(
                {"valid": all(r["valid"] for r in results), "results": results},
                indent=2,
                ensure_ascii=False,
                default=str,
            )
...
def run_tests() -> int:
    class TestValidateFrontmatter(unittest.TestCase):
        ...
        def test_nested_okf_metadata_is_preserved(self) -> None:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                self.assertEqual(
                    result["frontmatter"]["sources"][0]["id"], "policy"
                )
        ...
        def test_missing_pyyaml_reports_actionable_warning(self) -> None:
            result = subprocess.run(
                [sys.executable, "-S", __file__, "--test"],
                capture_output=True,
                text=True,
                check=False,
            )
    ...
    result = unittest.TextTestRunner(verbosity=2, stream=sys.stderr).run(
        unittest.TestLoader().loadTestsFromTestCase(TestValidateFrontmatter)
    )
