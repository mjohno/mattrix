...
def all_directories(bundle_root: Path) -> Iterable[Path]:
    ...
    while pending:
        ...
        try:
            ...
            raise OSError(f"cannot resolve directory {directory}: {exc}") from exc
...
def render_index(bundle_root: Path, directory: Path) -> str:
    ...
    dirs = sorted((child for child in children if child.is_dir() and not child.name.startswith(".")), key=lambda path: path.name)
    concepts = sorted((child for child in children if child.is_file() and child.suffix == ".md" and child.name not in RESERVED), key=lambda path: path.name)
    lines = [f"# {title}", ""]
    if dirs:
        lines.extend(["## Subdirectories", ""])
        lines.extend(f"* [{titleize(child.name)}]({child.name}/)" for child in dirs)
        lines.append("")
    if concepts:
        lines.extend(["## Concepts", ""])
        lines.extend(f"* [{titleize(concept.stem)}]({concept.name})" for concept in concepts)
        lines.append("")
...
def rebuild(bundle_root: Path, write: bool = False) -> dict[str, Any]:
    ...
    if errors:
        return {"bundle_root": str(bundle_root), "dry_run": not write, "written": written, "would_write": would_write, "errors": errors}
    if not write:
        ...
        for path, text in planned:
            try:
                ...
                errors.append({"path": str(path), "error": f"could not write index: {exc}"})
                break
    return {"bundle_root": str(bundle_root), "dry_run": not write, "written": written, "would_write": would_write, "errors": errors}


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Explicitly rebuild OKF index.md files.")
    parser.add_argument("bundle_root", nargs="?", help="MKF bundle root")
    parser.add_argument("--write", action="store_true", help="Write indexes; default is dry run")
...
def main(args: argparse.Namespace) -> int:
    ...
    for path in result["would_write"]: print(f"would write: {path}", file=sys.stderr)
    for path in result["written"]: print(f"written: {path}", file=sys.stderr)
    for error in result["errors"]: print(f"error: {error['path']}: {error['error']}", file=sys.stderr)
    ...
    class TestRebuildIndexes(unittest.TestCase):
        def test_default_is_dry_run_and_write_overrides_index(self) -> None:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                self.assertIn("concept.md", (root / "index.md").read_text(encoding="utf-8"))
        def test_follows_symlink_once_and_excludes_log(self) -> None:
            with tempfile.TemporaryDirectory() as temp_dir, tempfile.TemporaryDirectory() as linked_dir:
                ...
                self.assertIn("concept.md", text); self.assertNotIn("log.md", text); self.assertNotIn("generated by", text)
    result = unittest.TextTestRunner(verbosity=2, stream=sys.stderr).run(unittest.TestLoader().loadTestsFromTestCase(TestRebuildIndexes))
