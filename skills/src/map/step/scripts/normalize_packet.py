...
def task(value: Any, label: str = "packet", complete: bool = False) -> dict[str, Any]:
    ...
    if not isinstance(obj.get("slug"), str) or not SLUG.fullmatch(obj["slug"]):
        raise PacketError(f"{label}.slug must be lowercase kebab-case")
    if not isinstance(obj.get("intent"), str) or not obj["intent"].strip():
        raise PacketError(f"{label}.intent is required")
    ...
    if complete:
        ...
        if (
            not isinstance(obj["do"].get("summary"), str)
            or not obj["do"]["summary"].strip()
        ):
            raise PacketError(f"{label}.do.summary is required")
        if obj["validate"].get("result") not in RESULTS:
            raise PacketError(f"{label}.validate.result is invalid")
    return obj
...
def proposals(obj: dict[str, Any]) -> None:
    ...
    if not isinstance(packets, list):
        raise PacketError("proposed_next_packets must be a list")
    slugs = [
        task(x, f"proposed_next_packets[{i}]")["slug"] for i, x in enumerate(packets)
    ]
    if len(set(slugs)) != len(slugs):
        raise PacketError("proposed_next_packets contains duplicate slugs")
    rec = obj.get("recommendation")
    if rec is not None and rec not in slugs:
        raise PacketError("recommendation must name a proposed packet or be null")
...
def repeated(
    parser: argparse.ArgumentParser, option: str, *, required: bool = False, help: str
) -> None:
    parser.add_argument(
        option,
        action="append",
...
def add_complete_task_options(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "--slug", required=True, help="lowercase kebab-case task identifier"
    )
    ...
    repeated(
        ...
        "--criteria",
        required=True,
        help="observable acceptance criterion; repeat for each criterion",
    )
    ...
    repeated(
        parser, "--validate-evidence", help="validation evidence; repeat for each item"
    )
...
def complete_task_from(args: argparse.Namespace) -> dict[str, Any]:
    return {
        ...
        "validate": {
            "result": args.validate_result,
            "evidence": args.validate_evidence,
        },
    }
...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Build and validate a STEP role YAML packet"
    )
    ...
    repeated(
        coordinator,
        "--slug",
        help="proposed task identifier; repeat this option for each proposal",
    )
    repeated(
        coordinator,
        "--intent",
        help="proposed task outcome; repeat this option for each proposal",
    )
    coordinator.add_argument(
        ...
        default=[],
        metavar="TEXT",
        help="criteria for one proposal; repeat this option for each proposal",
    )
    coordinator.add_argument(
        "--recommendation", help="slug of the recommended proposal"
    )
    ...
    repeated(
        assessor, "--retro-issues", help="friction or failure; repeat for each item"
    )
    repeated(
        assessor,
        "--retro-actions",
        help="concrete next-step input; repeat for each item",
    )
    assessor.add_argument(
        "--clarification-needed",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="whether one worker clarification is needed",
    )
    return p
...
def packet_from(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.role == "worker":
        return {"packet": complete_task_from(args)}
    if args.role == "assessor":
        return {
            "current_packet": complete_task_from(args),
            "retro": {
                "wins": args.retro_wins,
                "issues": args.retro_issues,
                "actions": args.retro_actions,
            },
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.test:
        result = unittest.TextTestRunner(stream=sys.stderr, verbosity=2).run(
            unittest.defaultTestLoader.loadTestsFromTestCase(NormalizerTests)
        )
        return 0 if result.wasSuccessful() else 1
    try:
        print(
            yaml.safe_dump(validate(args.role, packet_from(args)), sort_keys=False),
            end="",
        )
