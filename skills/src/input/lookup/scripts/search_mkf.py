...
try:
    ...
    print(
        "PyYAML is required to search MKF bundles. Install PyYAML and rerun.",
        file=sys.stderr,
    )
    sys.exit(2)
...
def first_excerpt(
    ...
    pos = min(positions)
    return re.sub(
        r"\s+", " ", text[max(0, pos - width // 2) : pos + width // 2]
    ).strip()
...
def parse_advanced_filters(values: list[str]) -> dict[str, str]:
    ...
    for value in values:
        if ":" not in value:
            raise ValueError("advanced fields must use FIELD:VALUE")
        field, expected = value.split(":", 1)
        if field not in ADVANCED_FIELDS:
            raise ValueError(f"unsupported advanced field: {field}")
...
def search_file(
    ...
    for field, expected in advanced_filters.items():
        if expected not in text_value(frontmatter.get(field, "")).lower():
            return None
    cid = concept_id(bundle_root, path)
    path_hits = count_matches(
        " ".join(
            path.relative_to(bundle_root).as_posix().split("/")
            + [cid, path.stem]
        ),
        query_terms,
    )
    if path_hits:
        tier, fields, score, excerpt = (
            "path",
            ["directory", "filename", "concept_id"],
            300 + path_hits * 10,
            cid,
        )
    else:
        fields = [
            ...
            if count_matches(
                text_value(frontmatter.get(field, "")), query_terms
            )
        ]
        if fields:
            tier, score = (
            ...
            excerpt = "; ".join(
                f"{field}: {text_value(frontmatter[field])}" for field in fields
            )
        else:
            ...
            tier, fields, score, excerpt = (
                ...
                ["body"],
                100 + hits,
                first_excerpt(body, query_terms),
            )
...
def search_bundles(
    ...
    bundles: list[tuple[str, Path]],
    limit: int,
    advanced_filters: dict[str, str] | None = None,
) -> dict[str, Any]:
    ...
    for order, (name, root) in enumerate(bundles):
        ...
        for path in walk_concepts(root):
            try:
                ...
                errors.append(
                    {"bundle": name, "path": str(path), "error": str(exc)}
                )
                continue
            if match:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Search MKF/OKF bundles and return JSON metadata matches."
    )
    parser.add_argument("--query")
    parser.add_argument("--bundle", action="append", default=[])
    parser.add_argument("--limit", type=int, default=10)
    parser.add_argument(
        "--advanced-field", action="append", default=[], metavar="FIELD:VALUE"
    )
    ...
    try:
        args.advanced_filters = parse_advanced_filters(args.advanced_field)
...
def main(args: argparse.Namespace) -> int:
    ...
    output = search_bundles(
        args.query, bundles, args.limit, args.advanced_filters
    )
...
def run_tests() -> int:
    class TestSearchMkf(unittest.TestCase):
        def test_excludes_reserved_and_returns_invalid_error(self) -> None:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                self.assertEqual(
                    [item["concept_id"] for item in output["results"]], ["good"]
                )
        ...
        def test_type_only_and_advanced_filter(self) -> None:
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                output = search_bundles(
                    "needle", [("root", root)], 10, {"status": "stable"}
                )
    ...
    result = unittest.TextTestRunner(verbosity=2, stream=sys.stderr).run(
        unittest.TestLoader().loadTestsFromTestCase(TestSearchMkf)
    )
