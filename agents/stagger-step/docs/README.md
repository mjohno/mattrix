...
```bash
STEP_FILE=STEP-example.yaml python -m stagger_step.cli init --goal "Ship the change" --change artifacts --commit --packet_history 3 --coordinator-model gpt-5.6-terra --coordinator-thinking medium
...
Persisted state contains `version`, `goal`, root-level `role_settings`, optional `change_path`, `commit_mode`, `packet_history`, `lessons`, completed `history`, `current`, `next`, `recommended`, and `completed`. A new unstarted state has no `current`, `next`, or `recommended` value; it is distinct from a terminal state because `completed` is false. `role_settings` stores the fixed model and thinking level for each role. `packet_history` is a positive integer that limits detailed completed packets supplied to the Coordinator; `init --packet_history` sets it and defaults to `3`. Worker, Validator, and Assessor outputs are intentionally never persisted before Owner approval. Coordinator bootstrap output is persisted to create the first Owner gate; later Coordinator output remains pending until Owner approval. A gate contains `goal`, `lessons`, `history`, `current`, ranked `proposals`, `recommended`, and `completed`.
