...
```bash
STEP_FILE=STEP-example.yaml python -m stagger_step.cli init --goal "Ship the change" --reference "Flow: docs/flow.md" --reference "Values: docs/values.md" --reference "Specification: docs/spec.md" --reference "Plan: docs/plan.md" --reference "Rubric: docs/rubric.md" --change artifacts --commit --packet_history 3 --coordinator-model gpt-5.6-terra --coordinator-thinking medium
...
`init --reference TEXT` is repeatable and stores ordered root-level `references`. A reference is opaque Owner-provided shared context, such as a file path, labeled file path, specification, plan, rubric, execution policy, or lookup query. Stagger Step does not resolve, read, classify, or validate references. Every role receives the strings and uses applicable material with its available workspace and Pi capabilities. `gate` and `session` do not change references; manual STEP state updates are the escape hatch.

Use these optional labels when supplying known Markdown artifacts, in this order: `Flow: <path or source>`, `Values: <path or source>`, `Specification: <path or source>`, `Plan: <path or source>`, and `Rubric: <path or source>`. Labels are Owner guidance, not parsed types. Flow guides useful activity and return paths; Values ranks choices and resolves priority conflicts; Specification defines future state and scope; Plan identifies gap-closing work and dependencies; Rubric defines quality criteria. A known unmet criterion is `partial` or `failure`. Missing, contradictory, unavailable, undefined, or out-of-bounds governing guidance is `blocked`; the Coordinator recommends one bounded task that resolves it.
...
In `session`, enter `afk` at a manual gate to approve that gate and automatically approve later gates in the same process. AFK is never written to STEP state. A Validator `blocked` result or a Coordinator `blocked: true` response returns active AFK to manual mode before the next normal Owner gate. The Coordinator uses `blocked: true` when its recommended task resolves a coordination blocker. At any manual blocker gate, entering `afk` resumes AFK; it also clears the Coordinator blocker flag when present. `approved` clears that flag too. `break` and Ctrl+C at a manual response prompt leave the persisted blocker unchanged, while revision feedback lets the Coordinator set the next gate's blocker value. AFK also returns to manual mode when more than one of the last ten completed tasks is `failure`; before ten tasks complete, one failure is allowed. `partial` does not stop it. Ctrl+C while AFK returns to manual mode without clearing a blocker. From manual mode, `break` and Ctrl+C exit the session.

## State and gates

Owner gates render as Markdown on stdout. They show the goal, supplied references when present, applicable lessons and current-task results, ranked next tasks, and the recommendation, then prompt with `**Response:**`. A Coordinator blocker appears in this normal gate as a `## Coordination Blocker` section. It identifies a blocker-resolution task, not a task failure or a separate gate type. Submitted responses are echoed and followed by `---` before the next review. The recommended next task is marked `**RECOMMENDED**`. Empty sections are omitted. YAML remains the persisted STEP state format.

Persisted state contains `version`, `goal`, root-level `role_settings`, optional `change_path`, `commit_mode`, `packet_history`, ordered `references`, `lessons`, `coordination_blocked`, completed `history`, `current`, `next`, `recommended`, and `completed`. A new unstarted state has no `current`, `next`, or `recommended` value; it is distinct from a terminal state because `completed` is false. `role_settings` stores the fixed model and thinking level for each role. `packet_history` is a positive integer that limits detailed completed packets supplied to the Coordinator; `init --packet_history` sets it and defaults to `3`. Coordinator bootstrap output is persisted to create the first Owner gate. Later role output remains pending until Owner approval, revision, or a `session` exit from a displayed manual gate. `break` and Ctrl+C at that gate atomically persist the complete prepared gate without approving it, so a later session resumes the same Owner gate without rerunning roles. A gate contains `goal`, `references`, `lessons`, `history`, `current`, ranked `proposals`, `recommended`, and `completed`.

The Coordinator context contains the goal, references, lessons, Owner revision, `current_gate` for the completed packet awaiting approval and persistence with its Assessor retro actions, plus top-level proposals, recommendation, and completion state. It includes detailed `recent_history` for the newest `packet_history` completed packets and concise oldest-to-newest `history_index` entries for all older completed tasks. The coordinator selects durable gate lessons from this context. Approval validates the displayed gate, then atomically commits its completed current packet, coordinator-selected lessons, and recommended next packet (or terminal completion). The next `session` runs an approved active task through Worker → Validator → Assessor → Coordinator before returning to the human.

## Pi RPC adapter discovery

By default, the adapter starts an ephemeral Pi role session for each role in a STEP transition. It invokes Pi with a debuggable `--name STEP-<step-slug>-<task-slug>-<role>`, the role's persisted `--model` and `--thinking` values, and a random `--session-id`; the ID is reused for correction or clarification calls in that transition, but an idle timeout replaces it before replaying the task. It logs each session name and ID at INFO and never writes either to STEP state. The adapter loads the project-owned `pi-stagger-step` extension and selects one role through `--step-role`. It sends a JSONL `prompt`, waits for `agent_settled`, and accepts only the matching `stagger_step_finalize_<role>` tool result. That YAML is independently normalized through `stagger-step normalize --role <role>` before STEP processing. `--harness-session off` instead uses `--no-session`. A fresh subprocess is still started for each role invocation, role sessions remain distinct, and the adapter terminates the child before a user gate. If Pi rejects a role's model or thinking level at startup, the adapter exits with a critical error that identifies the role and settings; it does not change the STEP file.
