...
- A fresh process is created per role invocation, named `STEP-<slug>-<role>`. By default it reconnects to that role's deterministic STEP-file-specific UUID session, preserving same-role context while retaining role isolation. `--harness-session off` uses `--no-session`. The child environment removes `STEP_FILE` and `STAGGER_STEP_*`; no STEP path is supplied to Pi.
