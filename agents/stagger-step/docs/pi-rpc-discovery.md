...
- A fresh process is created per role invocation, named `STEP-<step-slug>-<task-slug>-<role>`. Each transition generates a random UUID for each role when first invoked; that ID is reused for correction or clarification calls in the same transition, but an idle timeout replaces it before replaying the task, then it is cleared before the next transition. The deterministic name and the random session ID preserve same-role context while retaining role isolation. The harness explicitly loads `pi-stagger-step` and supplies its selected `--step-role`. `--harness-session off` uses `--no-session`. The child environment removes `STEP_FILE` and `STAGGER_STEP_*`; no STEP path is supplied to Pi.
- Role output is collected only from the matching `stagger_step_finalize_<role>` `tool_execution_end` result and is accepted only after `agent_settled`. The result is passed through `stagger-step normalize --role <role>` before use. A nonblocking stream wait resets the current 30-, 60-, or 120-second idle timeout for every Pi JSONL event and also enforces a 30-minute maximum invocation duration. On either of the first two idle timeouts, the harness reaps the child, replaces the role session, and replays the unpersisted task; the third is terminal. Missing finalizers, malformed YAML, early closure, and timeout are diagnosable `HarnessError` values, never a state inference.
