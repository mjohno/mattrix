...
function parametersFor(role: Role) {
  ...
  return Type.Object(
    {
      wins: stringsSchema,
      issues: stringsSchema,
      actions: stringsSchema,
...
function canonicalInput(role: Role, params: Record<string, unknown>): Record<string, unknown> {
  if (role === "coordinator") {
    ...
    const slugs = params.proposals.map((proposal, index) => {
      ...
      nonEmptyString(item.slug, `proposals[${index}].slug`);
      nonEmptyString(item.intent, `proposals[${index}].intent`);
      nonEmptyStrings(item.criteria, `proposals[${index}].criteria`);
  ...
  if (role === "worker") {
    nonEmptyString(params.work_summary, "work_summary");
    nonEmptyStrings(params.work_evidence, "work_evidence");
    nonEmptyString(params.validation_summary, "validation_summary");
    nonEmptyStrings(params.validation_evidence, "validation_evidence");
    return {
      do: { summary: params.work_summary, evidence: params.work_evidence },
      validate: {
        result: params.result,
        summary: params.validation_summary,
        evidence: params.validation_evidence,
      },
    };
  }
  nonEmptyStrings(params.wins, "wins");
  nonEmptyStrings(params.issues, "issues");
  nonEmptyStrings(params.actions, "actions");
  if (typeof params.clarification_needed !== "boolean") {
    throw new Error("clarification_needed must be boolean");
  }
  return {
    retro: { wins: params.wins, issues: params.issues, actions: params.actions },
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", () => {
    ...
    pi.registerTool({
      ...
      description: `Validate, normalize, and finalize the ${role} STEP response.`,
      ...
      async execute(_toolCallId, params, signal) {
        try {
          const packet = canonicalInput(role, params as Record<string, unknown>);
          const canonical = await normalize(role, packet, signal);
          const parsed = JSON.parse(canonical);
          return {
            content: [{ type: "text", text: canonical }],
            details: { role, canonical: parsed },
          };
        } catch (error) {
          return {
