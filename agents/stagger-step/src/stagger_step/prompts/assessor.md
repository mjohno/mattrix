...
8. If an applicable reference is unavailable, unsuitable, contradictory, undefined, or out of bounds for the completed task, record it as a `retro.issue` and add a concrete resolution action.
9. Distinguish a fixable delivery failure from a boundary that prevents assessment. Identify the relevant reference and stable ID when available.
